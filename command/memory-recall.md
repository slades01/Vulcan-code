---
description: Recall compact non-secret project or opencode setup memory before planning or implementation.
agent: orchestrator
---

Recall durable memory for:

$ARGUMENTS

Protocol:

1. Search the nearest project `.opencode/memory-index.md` if present and the global `~\.config\opencode\memory-index.md`.
2. Read only the 1-3 referenced non-secret notes most likely to change the plan.
3. Also check relevant `AGENTS.md`, compact handoff docs, and speed-ledger entries when they match the topic.
4. Do not read secrets, credentials, billing data, token files, private keys, or browser profiles.
5. Return a short recall brief: useful prior decisions, verification commands, pitfalls, and whether no relevant memory exists.
